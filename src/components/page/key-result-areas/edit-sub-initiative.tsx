...
import { useGetDepartments, useGetMeasurementUnits, useGetSubInitiative } from "@/services/hooks/queries"
import { Loader } from "@/components/core/Button/Loader"
import { useFormikWrapper } from "@/hooks/useFormikWrapper"
import { AnimatePresence, motion } from "motion/react"
import { useEditSubInitiative } from "@/services/hooks/mutations"

interface EditSubInitiativeProps {
    id: string;
    isOpen: boolean;
    close: () => void;
}

export const EditSubInitiative: FC<EditSubInitiativeProps> = ({ isOpen, close, id }) => {
    const idPrefix = useId()
    const [parentIds, setParentIds] = useState("")
    const { data, isLoading } = useGetSubInitiative(id)
    const { mutate, isPending } = useEditSubInitiative(() => onClose())
    const { data: departmentBranches } = useGetDepartments({ parentID: parentIds, page_size: "10" })
    const { data: measurementUnits } = useGetMeasurementUnits({ page_size: Number.MAX_SAFE_INTEGER })

    const { errors, handleSubmit, register, resetForm, setFieldValue, values } = useFormikWrapper({
        initialValues: {
            assigned_weight: data?.assigned_weight || "",
            graded_weight: data?.graded_weight || "",
            kpi: data?.kpi ||  "",
            kra_id: data?.kra_id || "",
            name: data?.name || "",
            target: data?.target || "",
            unit_of_measurement_id: data?.unit_of_measurement_id ||  "",
            responsibilities: data?.responsibilities?.map((item) => ({ id: item?.id?.toString(), department_id: item?.department_id?.toString(), department_weight: item?.department_weight?.toString(), level: item?.level })) || [] as { id: string; department_id: string; department_weight: string; level: string; }[],
        },
        enableReinitialize: true,
        onSubmit() {
            const { responsibilities, assigned_weight, graded_weight, unit_of_measurement_id, kra_id, ...rest } = values
            ...
            mutate({ id, ...rest, assigned_weight: parseInt(assigned_weight as string), graded_weight: parseInt(graded_weight as string), unit_of_measurement_id: parseInt(unit_of_measurement_id as string), kra_id: parseInt(kra_id as string), responsibilities: newResponsibilities })
        },
    })
    ...
    useEffect(() => {
        const parentId = data?.responsibilities?.find((item) => item?.level === "division")
        const ids = [parentId?.parent_id?.toString(), ...values.responsibilities.map((item) => item?.department_id).filter((item) => !!item)].filter((item) => !!item)
        if (ids.length > 0) {
            setParentIds(ids.join())
            return;
        }
    },[data?.responsibilities, values.responsibilities])
    ...
    const deptDivisions = useMemo(() => {
        return departmentBranches?.filter((item) => item?.level === "division")?.map((item) => ({ label: item?.name, value: item?.id?.toString() })) || []
    },[departmentBranches])
                        ...
                        <form onSubmit={handleSubmit} className="relative flex flex-col gap-4 h-full w-full max-w-2xl mx-auto">
                            <div className="flex flex-col flex-1 gap-4 overflow-y-scroll scrollbar-hide">
                                <div className="flex items-center justify-between gap-2">
                                    <Drawer.Title className="font-semibold text-2xl text-gray-900">Edit Sub-Initiative</Drawer.Title>
                                ...
                                <div className="flex flex-col w-full gap-5">
                                    ...
                                    <div className="grid gap-4">
                                        ...
                                        <div className="flex items-start justify-end gap-3 p-3 rounded bg-green-secondary-10">
                                            ...
                                            <span className="flex-1 text-xs font-medium text-green-secondary-40">
                            ...
                            <BaseButton type="submit" size="small" theme="primary" variant="filled" disabled={isPending} loading={isPending} block>Create sub-initiative</BaseButton>
                        </form>
                        </RenderIf>
                        <RenderIf condition={isLoading}>
