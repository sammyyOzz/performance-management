...
import { DeleteObjective, EditObjective, ViewObjective } from "@/components/page/key-result-areas"

export const OfficersKraDescriptionPage = () => {
    ...
    const [toggleModals, setToggleModals] = useState({
        openDeleteObjective: false,
        openEditObjective: false,
        openViewObjective: false,
    })
    
    const toggleDeleteObjective = useCallback(() => {
        setToggleModals((prev) => ({
        ...prev,
        openDeleteObjective: !toggleModals.openDeleteObjective,
        }))
    }, [toggleModals.openDeleteObjective])
    
    const toggleEditObjective = useCallback(() => {
        setToggleModals((prev) => ({
        ...prev,
        openEditObjective: !toggleModals.openEditObjective,
        }))
    }, [toggleModals.openEditObjective])
    
    const toggleViewObjective = useCallback(() => {
        setToggleModals((prev) => ({
        ...prev,
        openViewObjective: !toggleModals.openViewObjective,
        }))
    }, [toggleModals.openViewObjective])
    ...
    const breadcrumbs = [
        { label: "Officers KRA", href: "/kra/officers" },
        { label: "KRA Description", href: "/kra/officers/1" },
    ]
    ...
    const columns = [
        {
            ...
            cell: () => {
                return (
                    <span className="line-clamp-2">Develop training workplan and programs</span>
                )
            }
        },
        {
        ...
        },
        {
            ...
            cell: () => {
                return (
                    <span className="line-clamp-2">Training</span>
                )
            }
        },
        {
            ...
            cell: () => {
                return (
                    <Menu>
                        ...
                        >
                            <MenuItem>
                                <button type="button" className="group flex w-full items-center text-green-primary-40 gap-2 rounded-lg py-1.5 px-3" onClick={() => toggleViewObjective()}>
                                    <Eye size="16" color="#003A2B"/>
                                    View
                                </button>
                            </MenuItem>
                            <MenuItem>
                                <button type="button" className="group flex w-full items-center text-green-primary-40 gap-2 rounded-lg py-1.5 px-3" onClick={() => toggleEditObjective()}>
                                    <Edit2 size="16" color="#003A2B"/>
                                    Edit
                                </button>
                            </MenuItem>
                            <MenuItem>
                                <button type="button" className="group flex w-full items-center text-red-40 gap-2 rounded-lg py-1.5 px-3" onClick={() => toggleDeleteObjective()}>
    ...
    return (
        <section className="flex py-3.5 px-5 md:px-8 lg:px-10 xl:px-12 2xl:px-0 page-height overflow-y-scroll">
                    ...
                    <div className="grid gap-5">
                        <div className="flex flex-col items-start gap-4 md:gap-0 md:flex-row md:items-center md:justify-between py-2 border-b border-b-[#DFE2E7]">
                            <div className="grid gap-0.5">
                                <h1 className="font-semibold text-black text-2xl">Capability Building and Talent Management</h1>
                                <p className="font-normal text-gray-500 text-sm">See the description of tasks and activities for a key result area</p>
                            </div>
                            <div className="flex items-center justify-end gap-3 p-3 rounded bg-green-secondary-10">
                                ...
                                <span className="text-xs font-medium text-green-secondary-40">
                                    Note: For this key result area to be considered done, all objectives status must be done
                                </span>
                            </div>
                        </div>
            ...
            <DeleteObjective isOpen={toggleModals.openDeleteObjective} close={() => toggleDeleteObjective()} />
            <EditObjective isOpen={toggleModals.openEditObjective} close={() => toggleEditObjective()} />
            <ViewObjective isOpen={toggleModals.openViewObjective} close={() => toggleViewObjective()} />
        </section>
    )
}