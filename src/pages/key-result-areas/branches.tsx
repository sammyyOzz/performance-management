...
import { EditBranchKra } from "@/components/page/key-result-areas"
...
export const BranchesKRAPage = () => {
    ...
    const branches = useMemo(() => {
        return department?.children?.filter((item) => item?.level === "branch") || []
    }, [department?.children])
    
    const { data: departmentBranches } = useGetSubInitiatives({ department_id: (branches || []).map((item) => item?.id).join(), page_size: "10" }, { enabled: isSuccess })
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: ({ row }: { row: any }) => {
                const item = row?.original as FetchedSubInitiative
                return (
                    <span className="line-clamp-2 capitalize">{item?.responsibilities?.filter((v) => v?.level === "branch")?.map((v) => v?.department_name)?.join(", ")}</span>
                )
            }
        },
        {
    ...
    return (
        <section className="flex flex-1 py-6 px-5 md:px-8 lg:px-10 page-height overflow-y-scroll bg-[#FFFFFF]">
            <div className="flex flex-col flex-1 gap-6 max-w-screen-2xl mx-auto">
                <div className="flex flex-col gap-3">
                    <div className="grid gap-1 py-2 border-b border-b-[#DFE2E7]">
                        <h1 className="font-semibold text-2xl text-black uppercase">{department?.name || 'Branches'} - BRANCHES</h1>
                        <p className="font-normal text-sm text-[#667185]">See the key result area of your branches.</p>
                    </div>
                    <RenderIf condition={branches.length > 0}>
                        <div className="flex flex-col gap-1 w-full">
                            ...
                            <div className="flex items-center gap-8 p-4 rounded-md border border-[#DFE2E7]">
                                {
                                    branches.map((item) =>
                                        <span key={item?.id} className="text-sm text-grey-40 capitalize">{item?.name || 'Unnamed Branch'}</span>
                                    )
                                }
                            </div>
                        </div>
            ...
            <EditBranchKra kra={toggleModals.activeKra as FetchedSubInitiative} department={department as FetchedDepartmentType} isOpen={toggleModals.openEditKra} close={() => toggleEditKra(null)} />
        </section>
    )
}